...
def tune_attention_layers(model, model_type):
...
def get_model_peft(model, method, **kwargs):
    if(method == 'fullft'):
        ...
        if(kwargs["encoder"] == 'vit_base'):
            model = vit_SSF.vit_base_patch16_224(pretrained=True)
        elif(kwargs["encoder"] == 'vit_large'):
